...
def lineplot(
    ...
    # if all_ticks:
    plt.xticks(np.arange(0, max(x) + 1, 1)) if x_max is None else plt.xticks(
        np.arange(0, x_max + 1, 1)
    )
    plt.yticks(np.arange(0, max(y) + 1, 1)) if y_max is None else plt.yticks(
        np.arange(0, y_max + 1, 1)
    )
    ax.invert_yaxis()
...
def get_xy_from_curve(curve: Curve):
    coords = [
        (curve.get_coordinate(i).x, curve.get_coordinate(i).y)
