...
class Clustering:
    ...
    def getnum_iterations(self):
        return self.num_iterations

    def clusterGrid(self, grid):
        print("Clustering Grid")
        print(
            "Min clusters: ", self.min_clusters, ", max Clusters: ", self.max_clusters
        )
        inertias = []
        map_inertia = {}
...
<<<<<<< HEAD:webviz_plugin_boilerplate/plugins/RV/Clustering.py
            
            if (accumulated_inertia/inertias_sum >= 0.65 ):
=======

            if accumulated_inertia / inertias_sum >= 0.85:
>>>>>>> develop:webviz_plugin_boilerplate/plugins/RV/clustering.py
                best_k = k
                break
